    ...
    <section className="py-32 px-6 md:px-10 max-w-7xl mx-auto border-t border-white/5">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        <div>
          ...
          <p className="text-lg text-white/50 lowercase max-w-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {MOCK_POSTS.map((post, i) => (
          <motion.a
            ...
            className="group relative aspect-[4/5] overflow-hidden rounded-3xl bg-neutral-900 border border-white/5"
          >
            <img 
              src={post.url} 
              alt={post.caption} 
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
              <p className="text-white text-sm line-clamp-2 lowercase leading-snug">
                {post.caption}
              </p>
            </div>
            {post.type === 'VIDEO' && (
